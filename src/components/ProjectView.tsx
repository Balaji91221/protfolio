    ...
    <div className="pt-16 lg:pt-32 px-4 lg:px-12">
          ...
          {topicAll.map((data, index) => {
            return (
              <input
                ...
                name="options"
                data-title={data}
                className="btn text-Balaji-800 dark:text-Balaji-100 bg-Balaji-100 dark:bg-Balaji-800 border-none dark:hover:bg-Balaji-600 hover:bg-Balaji-300"
      ...
      <div className="block sm:hidden">
        <div className="dropdown ml-2 mb-2">
          ...
          <ul
            tabIndex={0}
            className="dropdown-content menu p-2 shadow bg-Balaji-100 dark:bg-Balaji-800 rounded-box w-52"
          >
            {topicAll.map((data, index) => (
              <li key={index} onClick={() => setTopic(data)}>
                {
                  <button className={data === topic ? "bg-Balaji-500" : ""}>
                    {data}
                  </button>
                }
              </li>
            ...
            >
              <a href={link} target="_blank" rel="noopener noreferrer">
                ...
                <div className="card-body p-4">
                  ...
                  <p className="text-left text-Balaji-600 dark:text-Balaji-200 mb-4 md:text-sm">
                    {desc}
                  </p>
                  <div className="card-actions flex flex-wrap justify-start">
                    {tags.length > 0 &&
                      tags.map((tag, index) => (
                        <div
                          key={index}
                          className="badge bg-white badge-outline mr-2 mb-2 px-2 py-1 rounded-full border border-Balaji-400 text-Balaji-600 text-xs font-semibold"
                        >
                          {tag}
                        </div>
                      ))}
